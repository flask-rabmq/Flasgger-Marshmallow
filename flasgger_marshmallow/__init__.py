...
def data_schema(schema, data, is_request=True):
    """

    :param schema: marshmallow Schema
    :type schema: type[marshmallow.Schema]
    :param data: flask data
    :type data: dict
    :param is_request: is flask reqeust default True
        ...
        for rule, verbs in specs:
            ...
            for verb, swag in verbs:
                ...
                defs += extract_definitions(
                    defs, endpoint=rule.endpoint, verb=verb, prefix_ids=prefix_ids
                )
                ...
                defs += extract_definitions(
                    params, endpoint=rule.endpoint, verb=verb, prefix_ids=prefix_ids
                )
    ...
    def decorator(func):
        def limit_log_length(content):
            ...
            if max_length_log and len(current_content.__str__()) > max_length_log:
                ...
                return "%s...%s" % (
                    current_content[: int(max_length_log / 2)],
                    current_content[-int(max_length_log / 2) :],
                )
        ...
        func.__doc__ = (
            (func.__doc__.strip() + generate_doc()) if func.__doc__ else generate_doc()
        )
        ...
        def wrapper(*args, **kw):
            ...
            logger.info(
                "request params\npath params: %s\nquery params: %s\nform params: %s\njson params: %s\n",
                log_format(path_params),
                log_format(query_params),
                log_format(form_params),
                log_format(json_params),
            )
            ...
            try:
                path_schema and setattr(
                    request, "path_schema", data_schema(path_schema, path_params)
                )
                query_schema and setattr(
                    request, "query_schema", data_schema(query_schema, query_params)
                )
                form_schema and setattr(
                    request, "form_schema", data_schema(form_schema, form_params)
                )
                json_schema and setattr(
                    request, "json_schema", data_schema(json_schema, json_params)
                )
            ...
            try:
                if response_schema and response_schema.get(code):
                    data = data_schema(
                        response_schema.get(code), data, is_request=False
                    )
                    ...
                    if r_headers_schema:
                        headers = data_schema(
                            r_headers_schema, headers, is_request=False
                        )
            except Exception as e:
                return (
                    "response error: %s"
